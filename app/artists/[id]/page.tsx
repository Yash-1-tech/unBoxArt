    ...
    <div>
      <div className="max-w-[1280px] mx-auto px-4 lg:px-8 py-8">
        <nav className="text-xs text-gray-400 mb-6 flex items-center gap-1.5">
          ...
          <Link href="/artists" className="hover:text-[#e63329]">Artists</Link>
          <ChevronRight size={12} />
        ...
        <div className="flex gap-8 border-b border-gray-100 mb-8">
          {['About Artist', 'Reviews', 'News'].map((tab) => {
            ...
            return (
              <button key={tab} onClick={() => setActiveTab(key)} className={`pb-3 text-xs font-semibold uppercase tracking-wider transition-colors border-b-2 -mb-px ${activeTab === key ? 'border-[#e63329] text-[#e63329]' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
                {tab}
              </button>
            );
          })}
        ...
        <div className="flex flex-col sm:flex-row gap-6 mb-8">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between gap-4 flex-wrap mb-2">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 tracking-wide">{ARTIST.name}</h1>
            ...
            <p className={`text-xs text-gray-500 leading-relaxed max-w-3xl ${!showFullBio ? 'overflow-hidden' : ''}`} style={!showFullBio ? { display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' } : {}}>
              {ARTIST.bio}
            </p>
            <button onClick={() => setShowFullBio(!showFullBio)} className="text-xs text-[#e63329] mt-1 hover:underline">
