...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!userId || !rating) return NextResponse.json({ error: 'userId and rating are required' }, { status: 400 });
    ...
    const alreadyReviewed = artwork.reviews.some((r) => r.user.toString() === userId);
    if (alreadyReviewed) return NextResponse.json({ error: 'You have already reviewed this artwork' }, { status: 400 });
