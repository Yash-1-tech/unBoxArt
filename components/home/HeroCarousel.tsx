  ...
  return (
    <section className="relative w-full h-[480px] md:h-[560px] lg:h-[640px] overflow-hidden bg-gray-900">
      ...
      <div className="relative h-full flex flex-col justify-end pb-16 px-6 lg:px-20 max-w-[1280px] mx-auto">
        <div key={current}>
          <h1 className="text-white text-3xl md:text-5xl lg:text-6xl font-bold leading-tight whitespace-pre-line mb-4 drop-shadow-sm">
            {slide.title}
          </h1>
      ...
      <div className="absolute bottom-6 left-6 lg:left-20 flex items-center gap-2">
        {slides.map((_, i) => (
          <button
            key={i}
            onClick={() => goTo(i)}
            className={`transition-all duration-300 rounded-full ${i === current ? 'bg-[#e63329] w-6 h-2' : 'bg-white/60 w-2 h-2 hover:bg-white'}`}
            aria-label={`Go to slide ${i + 1}`}
          />
        ))}
      </div>

      <button onClick={prev} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/30 hover:bg-black/50 text-white flex items-center justify-center transition-colors" aria-label="Previous slide">
        <ChevronLeft size={20} />
      </button>
      <button onClick={next} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/30 hover:bg-black/50 text-white flex items-center justify-center transition-colors" aria-label="Next slide">
